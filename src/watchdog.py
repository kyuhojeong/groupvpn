        ...
        for sock in socks[0]:
            ...
            if data[0] == '{':
                ...
                # we ignore connection status notification for now
                    ...
                    ip4 = gvpn.get_ip4(msg["uid"], self.state["_ip4"])
    ...
    def run_server(self):
        ...
        while run_event.is_set():
            try: 
                ...
                attempt += 1
                logging.debug("TinCan Failed {0} times".format(attempt));
                os.kill(tincan_process.pid, signal.SIGTERM)
                time.sleep(1)
                if attempt > 3:
                    logging.debug("TinCan Failed beyond threshold point");
                    run_event.clear()
                    break
...
def main():
    ...
    if os.path.exists(tincan_path):
        ...
        logging.debug("tincan binary doesn ot exist at specified directory")
        sys.exit(0)
